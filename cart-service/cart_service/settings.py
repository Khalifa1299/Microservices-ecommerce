...
Owns: cart app (cart, wishlist, coupons)
Port: 8004
"""
...
DATABASES = {
    'default': {
        ...
        'NAME': config('DB_NAME', default='cart_db'),
        'USER': config('DB_USER'),
...
# No AUTH_USER_MODEL — user_id stored as PositiveIntegerField
...
# --- Cache (Redis DB 5 — no Celery in cart-service) ---
CACHES = {
    'default': {
        'BACKEND': 'django_redis.cache.RedisCache',
        'LOCATION': config('REDIS_URL', default='redis://redis:6379/5'),
        'OPTIONS': {
            'CLIENT_CLASS': 'django_redis.client.DefaultClient',
        },
        'KEY_PREFIX': 'cart',
        'TIMEOUT': 1800,  # 30 minutes — cart changes frequently, shorter than product
    }
}
...
LOGGING = {
    ...
    'formatters': {
        'verbose': {'format': '%(asctime)s [cart-service] %(levelname)s %(message)s'},
    },
    'handlers': {
...
_provider = TracerProvider(resource=Resource.create({SERVICE_NAME: "cart-service"}))
